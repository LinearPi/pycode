...
train_img_batch, train_label_batch = tf.train.batch(
    [train_img, train_label], batch_size=10, capacity=200, num_threads=6)
train_label_batch = tf.expand_dims(train_label_batch, -1)  # expand dim

val_img, val_label = read_record('val.tfrecord', 224, 224)
val_img_batch, val_label_batch = tf.train.batch(
    [val_img, val_label], batch_size=10, capacity=200, num_threads=6)
...
def conv_bias_relu(input_tensor, scope_name, ind):
    with tf.variable_scope(scope_name):
        init = tf.constant_initializer(
            value=weight_list[ind], dtype=tf.float32)
        kernel = tf.get_variable(
            'kernel', shape=weight_list[ind].shape, dtype=tf.float32, initializer=init)
        conv = tf.nn.conv2d(input_tensor, kernel, [
...
def average_pool(input_tensor, scope_name):
    with tf.variable_scope(scope_name):
        pool = tf.nn.avg_pool(input_tensor, ksize=[1, 2, 2, 1], strides=[
...
def conv_bias_relu_init(input_tensor, scope_name, shape):
    with tf.variable_scope(scope_name):
        ...
        conv = tf.nn.conv2d(input_tensor, kernel, [
                            1, 1, 1, 1], padding='SAME', name='conv')
        bias = tf.add(conv, tf.constant(
            0, dtype=tf.float32, shape=[shape[-1]]), name='bias')
        relu = tf.nn.relu(bias, name='relu')
    return relu
...
def deconv(input_tensor, scope_name, shape, out_shape, strider=2):
    with tf.variable_scope(scope_name):  # tf.variable_scope(scope_name)
        kernel = tf.Variable(tf.truncated_normal(shape=shape, stddev=0.02))
        deconv = tf.nn.conv2d_transpose(input_tensor, kernel, out_shape, [
                                        1, strider, strider, 1], padding='SAME')
        bias = tf.add(deconv, tf.constant(0, dtype=tf.float32,
                                          shape=[deconv.shape[3].value]), name='bias')
...
def summary_activition(var):
    ...
    tf.summary.scalar(var.op.name+'/sparsity',
                      tf.nn.zero_fraction(var))  # 返回0在value中的小数比例
...
def fcn_net(img, gt, keep_prob):
    ...
    deconv1 = deconv(relu_dropout7, 'deconv1', [
                     4, 4, pool4.shape[3].value, 4096], tf.shape(pool4))
    fuse_1 = tf.add(deconv1, pool4, name="fuse_1")

    deconv2 = deconv(fuse_1, 'deconv2', [
                     4, 4, pool3.shape[3].value, pool4.shape[3].value], tf.shape(pool3))  # .value
    fuse_2 = tf.add(deconv2, pool3, name="fuse_2")

    shape = tf.shape(img)
    deconv_shape3 = tf.stack([shape[0], shape[1], shape[2], NUM_OF_CLASSESS])
    deconv3 = deconv(fuse_2, 'decon3', [
                     16, 16, NUM_OF_CLASSESS, pool3.shape[3].value], deconv_shape3, strider=8)
...
def train():
    ...
    img = tf.placeholder(dtype=tf.float32, shape=[
                         None, IMAGE_SIZE, IMAGE_SIZE, 3])
    gt = tf.placeholder(dtype=tf.int32, shape=[
                        None, IMAGE_SIZE, IMAGE_SIZE, 1])
    keep_prob = tf.placeholder(dtype=tf.float32)
    pre, deconv3 = fcn_net(img, gt, keep_prob)
    ###############loss calculate####################
    loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=deconv3, labels=tf.squeeze(gt, squeeze_dims=3)))
    tf.summary.scalar('loss', loss)
    ##############optimizer##########################
    op = tf.train.AdamOptimizer(FLAGS.learning_rate).minimize(
    ...
    with tf.Session() as sess:
        ...
        try:
            for ep in range(1, NUM_EPOCHES):
                temp_train_img, temp_train_label = sess.run(
                    [train_img_batch, train_label_batch])
                processed_img_batch = pre_process(temp_train_img)
                _, summary, predicts_value, train_loss = sess.run([op, merged, pre, loss], feed_dict={
                                                                  img: processed_img_batch, gt: temp_train_label, keep_prob: 0.85})
                train_precision = np.mean(predicts_value == temp_train_label)
                train_writer.add_summary(summary, ep)
    #            print('epoch '+str(ep)+':  ',train_loss)
                # print('----------------------------',i,'--------------------------')
                count = ep % 500
                mes = ('>>Step: %d loss = %.4f acc = %.3f' %
                       (ep, train_loss, train_precision))
                view_bar(mes, count, 500)
                if ep % 500 == 0 and ep != 0:
                    temp_val_img, temp_val_label = sess.run(
                        [val_img_batch, val_label_batch])
                    processed_val_img_batch = pre_process(temp_val_img)
                    _, summary, val_predicts, val_loss = sess.run([op, merged, pre, loss], feed_dict={
                                                                  img: processed_val_img_batch, gt: temp_val_label, keep_prob: 0.85})
                    ...
                    logging.info('>>%s Saving in %s' %
                                 (datetime.datetime.now(), FLAGS.checkpoint))
                    ...
                    if ep % 100 == 99:
                        ...
                        summary, _ = sess.run([merged, op], feed_dict={img: processed_img_batch, gt: sess.run(
                            train_label_batch), keep_prob: 0.85}, options=run_options, run_metadata=run_metadata)
                        train_writer.add_run_metadata(
                            run_metadata, 'step%03d' % ep)
            train_writer.close()
            val_writer.close()
            coord.request_stop()
            coord.join(threads)
...
def save_result(pre_batch, gt_batch, ep):
    ...
    for i in range(img_shape[0]):
        ...
        misc.imsave('test_result/pre/pre' +
                    str(ep*FLAGS.batchsize+i)+'.png', temp_pre)
        temp_gt = gt_batch[i].reshape([224, 224])
        misc.imsave('test_result/gt/gt' +
                    str(ep*FLAGS.batchsize+i)+'.png', temp_gt)
...
def test():
    ...
    with tf.Session() as sess:
        ...
        for ep in range(NUM_EPOCHES):
            temp_test_img, temp_test_label = sess.run(
                [val_img_batch, val_label_batch])
            processed_test_img_batch = pre_process(temp_test_img)
            _, predict_label = sess.run([op, pre], feed_dict={
                                        img: processed_test_img_batch, gt: temp_test_label, keep_prob: 1.0})
